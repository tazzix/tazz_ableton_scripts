...
class MPK_mini_shift(ControlSurface):
    __module__ = __name__
    __doc__ = " MPK_mini_shift controller script "

    def __init__(self, c_instance):
        ControlSurface.__init__(self, c_instance)
        self._suppress_session_highlight = True
        self._suppress_send_midi = True  # Turn off rebuild MIDI map until after we're done setting up
        self.log_message(time.strftime("%d.%m.%Y %H:%M:%S", time.localtime()) + "--------------= MPK_mini_shift log opened =--------------") # Writes message into Live's main log file. This is a ControlSurface method.
    ...
    def _setup_session_control(self):
        ...
        self.session = SpecialSessionComponent(self, num_tracks, num_scenes, MIDI_CC_TYPE, CHANNEL, HSCROLLENCODER, VSCROLLENCODER, False)
        ...
        # --SHIFT ONLY-- self.session.set_track_bank_buttons(ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, session_right), ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, session_left)) # (right_button, left_button) This moves the "red box" selection set left & right. We'll use the mixer track selection instead...
    ...
    def __update_matrix(self):
        a=1
        u"""for scene_index in range(num_scenes):
            ...
            for track_index in range(num_tracks):
                ...
                #else:
                #    if button != None: button.turn_off()
    ...
    def _clear_controls(self):
        ...
        self.session.set_selected_scene_launch_button(None)
        ...
        self.session.set_ignore_encoders(True)
        # MIXER
        ...
        for track_index in range(num_tracks):
            ...
            strip.set_send_controls(None) #tuple(self.mixer.send_controls))
    ...
    def _set_initial_mode(self):
        is_momentary = True
        ### MIXER
        self.mixer.master_strip().set_volume_control(SliderElement(MIDI_CC_TYPE, CHANNEL, mixer_mastervol_cc))
        # DEVICE Right/Left, ON/OFF, LOCK AND PARAMETERS
        ...
        device_param_controls = []
        onoff_control = ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, device_onoff)
        setlock_control = ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, device_lock)
        for index in range(num_device_params):
            knob = None
        ...
        self.session.set_ignore_encoders(False)
        self.session.set_stop_all_clips_button(ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, session_stopall_cc))
        self.session.set_slot_launch_button(ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, sel_clip_launch))
        self.session.set_selected_scene_launch_button(ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, sel_scene_launch))
        # TRANSPORT
        self.transport.set_metronome_button(ButtonElement(not is_momentary, MIDI_CC_TYPE, CHANNEL, transport_metronome_cc))
        self.transport.set_overdub_button(ButtonElement(not is_momentary, MIDI_CC_TYPE, CHANNEL, transport_overdub_cc))
        self.transport.set_loop_button(ButtonElement(not is_momentary, MIDI_CC_TYPE, CHANNEL, transport_loop_cc))
        self.transport.set_record_button(ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, transport_record_cc))


    def _set_initial_mode_org(self):
        is_momentary = True
        ### MIXER
    ...
    def _set_shift_mode(self):
        ...
        for index in range(num_tracks):
            ...
            self.mixer._update_send_index(self.mixer.sends_index)
            strip.set_volume_control(SliderElement(MIDI_CC_TYPE, CHANNEL, mixer_volumefader_cc[index]))
            # TODO: fix overlap in session control
            #if index < 3: 
            self.mixer.send_controls[self.mixer.sends_index] = EncoderElement(MIDI_CC_TYPE, CHANNEL, mixer_sendknob_cc[index], Live.MidiMap.MapMode.absolute)
    ...
    def _set_alt_mode(self):
        ...
        stop_track_controls = []
        self.session.set_ignore_encoders(False)
        ## SESSION navigation and stop all
        ...
        #self.session.set_stop_all_clips_button(ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, transport_record_cc))
        # SESSION STOP ALL CLIPS AND SCENE LAUNCH
        self.session.set_stop_all_clips_button(ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, stop_all))
        for index in range(num_scenes):
            self.session.scene(index).set_launch_button(ButtonElement(is_momentary, MIDI_CC_TYPE, CHANNEL, alt_scenelaunch_cc[index]))
    ...
    def _manage_modes(self, mode_index):
        self.log_message("NANOSHIFT: mode: %d", mode_index)
        if mode_index == 0:
    ...
    def disconnect(self):
        ...
        self.log_message(time.strftime("%d.%m.%Y %H:%M:%S", time.localtime()) + "--------------= MPK_mini_shift log closed =--------------") #Create entry in log file
        self._clear_controls()
        self.session = None
        self.mixer = None
        self.view = None
    ...
    # TODO: copy rest of the padlight functionality and test
